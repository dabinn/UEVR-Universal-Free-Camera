...
# 主要功能
* **自由攝影機**：5自由度控制，全向移動及2軸旋轉。 (控制器熱鍵切換)
* **三種飛行模式**：TPS水平/TPS全向/Space全向。 (預設為TPS水平模式)
* **三種跟隨模式**：跟隨/鎖定/只跟隨位置。 (預設為跟隨模式)
* **十段速度控制**：可適應各類型遊戲。
* **視野切換/儲存**：隨時切換/儲存視野。
* **環繞攝影機**：支援客製化插件使用。
* **時間變速**：暫停/慢動作/快動作。 **(新功能! v1.3.0)**
* **自定義熱鍵**：避免與遊戲按鍵衝突。
* **自定義選項**：依照需求調整插件設定。
* **進階安裝**：一次安裝，所有遊戲通用。 **(新功能! v1.3.0)**
...
慢動作搭配自由攝影機使用特別有趣，快動作則可以讓你快速通過一些無聊的過場動畫。暫停的功能則是能讓時間停止，特別適合用在遊戲不支援暫停的情況下，或是想要拍照時使用。  

附帶：**快轉自動剎車功能**。過場動畫結束自動停止快轉，以免進關卡時一不小心被打死。因為每個遊戲的過場動畫設計方式不同，這功能只有在成功辨識到過場動畫時才會生效。
...
# 進階安裝
如果你想要一次安裝，所有遊戲都能使用這插件，請參考以下步驟：
![](img/universal-installation.png)
1. 打開UEVR介面，按下`Open Golbal Dir`  
1. 建立一個名為`UEVR`的資料夾，然後在裏面建立一個名為`scripts`的資料夾。
1. 把解壓縮後的檔案複製到`scripts`目錄  
1. 這樣就好啦

* 注意1：這安裝方式必需搭配UEVR nightly版本才能使用，目前穩定版UEVR還不支援這功能。
* 注意2：插件自帶防止重覆載入的設計，如果在遊戲目錄下也有這插件的話，會以遊戲目錄的為主，這樣可確保與自定義插件的相容性。
